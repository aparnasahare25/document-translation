...
PPTX_LLM_MAX_WORKERS        = int(os.getenv("PPTX_LLM_MAX_WORKERS", "6"))
PPTX_TOP_K_PARAGRAPHS       = int(os.getenv("PPTX_TOP_K_PARAGRAPHS", "5"))
...
def _llm1_refine(
    ...
    *,
    ...
    """Ask Azure OpenAI to improve the MT output for one paragraph."""
...
# ─────────────────────────────────────────────────────────────────────────────
# TranslatorService  — mirrors scripts/translator_service_word.py, adapted for
#                      PowerPoint (.pptx) paragraph-level translation
# ─────────────────────────────────────────────────────────────────────────────
class TranslatorService:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Full 3-stage pipeline  (MT → LLM1 → RAG+LLM2)
    # ─────────────────────────────────────────────────────────────────────────
    def batch_translate_with_pipeline(
        ...
        *,
        batch_size: int = 100,
        max_workers: int = PPTX_LLM_MAX_WORKERS,
        top_k_paragraphs: int = PPTX_TOP_K_PARAGRAPHS,
        rolling_context_size: Optional[int] = 3,
    ) -> List[Optional[str]]:
        """
        Full 3-stage translation pipeline for PowerPoint (.pptx) paragraphs:

            Stage 1  : Azure Translator (MT, batched 100/call, same as batch_translate)
            Stage 2  : LLM1: Azure OpenAI grammar/fluency refinement
                         • Rolling context window (src + MT)
        ...
        Args:
            texts         : Original source paragraph strings (all paragraphs
                            collected across slides/tables/charts/notes).
            ...
            top_k_paragraphs : Top-K glossary paragraphs to retrieve per item in Stage 3.
        ...
        # ── pre-compute skip mask (same heuristics as PDF/Word pipelines) ────
        skip_mask = [_should_skip(t) for t in texts]
        skip_count = sum(skip_mask)
        print(f"[PPTX-PIPELINE] Total paragraphs: {n}, skipping {skip_count} (empty/no-alpha/URL)")

        # ─────────────────────────────────────────────────────────────────────
        # Stage 1 — MT  (existing batch_translate, unchanged)
        # ─────────────────────────────────────────────────────────────────────
        print(f"\n[PPTX-PIPELINE][STAGE 1 - MT] Translating {n} paragraphs (batch_size={batch_size}) ...")
        ...
        for i in range(n):
            if skip_mask[i]:
                ...
                mt_out[i] = texts[i]
        print(f"[PPTX-PIPELINE][STAGE 1 - MT] Done in {time.perf_counter() - t0:.2f}s")
        ...
        # Only refine items where MT actually changed something
        llm1_indices = [
            i for i in range(n)
            if not skip_mask[i] and mt_out[i] != texts[i]
        ]
        print(
            f"\n[PPTX-PIPELINE][STAGE 2 - LLM1] Candidates: {len(llm1_indices)} "
        ...
        def _llm1_task(i: int) -> Tuple[int, str, float]:
            ...
            # Short mode: very short text (likely a bullet/label/title on a slide)
            ...
            try:
                ...
                with stats_lock:
                    llm1_failures += 1
                print(f"[LLM1] item i={i} failed: {e} → fallback to MT")
                return i, mt_out[i], time.perf_counter() - t_start  # type: ignore[return-value]

        t0 = time.perf_counter()
        if llm1_indices:
            print(f"[LLM1] Refining {len(llm1_indices)} paragraphs with {max_workers} workers ...")
        ...
        print(
            f"[PPTX-PIPELINE][STAGE 2 - LLM1] Done in {time.perf_counter() - t0:.2f}s "
        ...
        # Only refine items where LLM1 output differs from source (translation happened)
        llm2_indices = [
            i for i in range(n)
            if not skip_mask[i] and pass1[i] != texts[i]
        ]
        print(
            f"\n[PPTX-PIPELINE][STAGE 3 - LLM2+RAG] Candidates: {len(llm2_indices)} "
        ...
        def _llm2_task(i: int) -> Tuple[int, str, float]:
            ...
            try:
                ...
                with stats_lock:
                    llm2_failures += 1
                print(f"[LLM2] item i={i} failed: {e} → keep LLM1")
                return i, cur, time.perf_counter() - t_start

        t0 = time.perf_counter()
        if llm2_indices:
            print(f"[LLM2] Refining {len(llm2_indices)} paragraphs with {max_workers} workers ...")
            with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        print(
            f"[PPTX-PIPELINE][STAGE 3 - LLM2+RAG] Done in {time.perf_counter() - t0:.2f}s "
            f"(failures={llm2_failures})"
        )

        total_wall = time.perf_counter() - t_pipeline_start
        print(f"\n[PPTX-PIPELINE] Complete. Total wall time: {total_wall:.2f}s")
